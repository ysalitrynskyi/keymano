//! model → `.keylayout` XML (. Stable, diff-friendly output.
//!
    ...
    for set in &kb.keymap_sets {
        ...
        for map in maps {
            ...
            if keys.is_empty() {
                ...
                // dummy key with the sentinel code 512 (uses Apple convention:
...
/// synthesizing an empty one if the model lacks it. Ensures
...
mod tests {
    ...
    fn action_always_emits_state_none_when_first() {
        ...
        // and place it before the others (format convention).
